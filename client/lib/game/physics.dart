...
class Physics {
  static bool detectBallBandCollision(
    ...
    return d <= ball.radius + buffer &&
        currentDistance <= ball.radius + buffer; // Added currentDistance check
  }
  ...
  static void resolveBallBandCollision(
    ...
    final closestPoint =
        p1 +
        segment *
            (((ball.position - p1).dot(segment) / segment.dot(segment)).clamp(
              0.0,
              1.0,
            ));
    ...
    final v2 =
        (band.velocities[segmentIndex] + band.velocities[segmentIndex + 1]) / 2;
    ...
    band.velocities[segmentIndex] -=
        normal * (impulseMagnitude / (2 * band.mass));
    band.velocities[segmentIndex + 1] -=
        normal * (impulseMagnitude / (2 * band.mass));
  ...
  static void resolveBallPostCollision(
    ...
    final impulseMagnitude =
        -(1 + 0.5) * relativeVelocity.dot(normal) / (1 / ball.mass);
