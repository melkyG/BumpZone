...
class ElasticBand {
  ...
  void handleBallCollision(Ball ball, double deltaT) {
    ...
    for (int i = 0; i < numPts - 1; i++) {
      ...
      if (Physics.detectBallBandCollision(ball, p1, p2, deltaT, 5.0)) {
        final segment = p2 - p1;
        final t = ((ball.position - p1).dot(segment) / segment.dot(segment))
            .clamp(0.0, 1.0);
    ...
    if (closestSegmentIndex != null && closestPoint != null) {
      ...
      Physics.resolveBallBandCollision(
        ball,
        this,
        closestSegmentIndex,
      ...
      if (deltaV.dot(outwardAxis) > 0) {
        final outwardComponent =
            outwardAxis.normalized() * deltaV.dot(outwardAxis);
  ...
  void update(double deltaT, Ball? ball) {
    ...
    if (ball != null) {
      ...
      for (int i = 0; i < posts.length; i++) {
        if (Physics.detectBallPostCollision(ball, posts[i], deltaT, 5.0)) {
      ...
      if (closestPost != null && closestPostIndex != null) {
        Physics.resolveBallPostCollision(
          ball,
          closestPost,
